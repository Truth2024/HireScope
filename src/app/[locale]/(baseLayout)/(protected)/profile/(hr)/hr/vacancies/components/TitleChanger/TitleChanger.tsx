...
export const TitleChanger = observer(({ initialTitle, setTitle, error }: TitleChangerProps) => {
  ...
  if (!isEditing) {
    return (
      <div className="flex items-center justify-between">
        {!initialTitle ? (
          <h1 className="text-lg  text-gray-400 italic">{t('titleEmpty')}</h1>
        ) : (
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900">{initialTitle}</h1>
        )}
    ...
    <div className="space-y-4">
      <div>
        <Input
          ...
          value={initialTitle}
          onChange={setTitle}
          placeholder={t('placeholderTitleChanger')}
          autoFocus
        />
